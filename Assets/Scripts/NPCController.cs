...
public class NPCController : MonoBehaviour
{
    ...
    // Animation root cache (for restoring after lying)
    ...
    // =========================================================
    // Animation root helpers (used by lying system)
    // =========================================================
    private void ResolveAnimationRoot()
    {
        _animationRoot = (animationController != null) ? animationController.transform : null;
    ...
    void Update()
    {
        ...
        {
            // Sitting and Lying bypass the agent-ready check since they manage the agent themselves
    ...
    // =========================================================
    // Sitting public API  (unchanged from base)
    // =========================================================
    ...
    void TickFSM(float dt)
    {
        ...
        if (!_hasCommand && _state != NPCState.Talk && _state != NPCState.Sitting && _state != NPCState.Lying)
        {
    ...
    // =========================================================
    // SITTING  (100% unchanged from base)
    // =========================================================
    ...
    private bool TryAcquireSeat()
    {
        ...
        for (int i = 0; i < seats.Length; i++)
        {
            ...
            {
                if (seatDebugLogs)
                    Debug.Log($"{name} Sit: REJECT '{s.name}' reason=navmesh_sample_failed");
                continue;
            }
        ...
        if (seatDebugLogs)
            Debug.Log($"{name} Sit: ACQUIRED '{_seat.name}'");
    ...
    private void TickApproachFront()
    {
        ...
        {
            if (seatDebugLogs)
                Debug.Log($"{name} Sit: Arrived at pre-sit point. Transitioning to Aligning.");
    ...
    private void BeginSitDown()
    {
        ...
        // Do NOT snap XZ to the seat transform — the backstep already placed the NPC correctly.
        // Snapping XZ to seatTf.position moves the NPC into the seat mesh and causes clipping.
        // Just lock rotation to face the seat.
        if (_seatTf != null)
    ...
    // =========================================================
    // LYING / BED  (new — mirrors sitting structure)
    // =========================================================
    ...
    // Direction the NPC faces when standing at the foot (and when lying):
    // from the lie marker toward the foot marker = facing away from the head of the bed.
    ...
    private void SnapToBedLyingPoseNow()
    {
        ...
        {
            transform.position = (_bedLieTf != null) ? _bedLieTf.position : _bedLieWorldPos;
        }
    ...
    private bool TryAcquireBed()
    {
        ...
        foreach (var b in beds)
        {
            ...
            Vector3 d = bedPos - _spawnPoint; d.y = 0f;
            if (d.sqrMagnitude > radius * radius) continue;
    ...
    private void TickApproachBedFront()
    {
        ...
        float planarDist = new Vector2(
            transform.position.x - _preLieNavPos.x,
            transform.position.z - _preLieNavPos.z).magnitude;
    ...
    private void BeginLieDown()
    {
        ...
        // Position at foot marker facing away from bed
        if (_bedFootTf != null)
            transform.position = _bedFootTf.position;

        transform.rotation = Quaternion.LookRotation(GetBedFacing(), Vector3.up);
    ...
    private void TickLieDownPlaying(float dt)
    {
        ...
        // Fallback: if trigger didn't land, force-play LieDown
        ...
        // Transition to LyingIdle when LieDown is complete (or animator already moved there)
        bool lieDownFinished =
            inLieIdle ||
            (inLieDown && !animationController.IsInTransition(lieAnimLayer) && info.normalizedTime >= 0.95f);

        // Safety timeout — generous ceiling so snap never fires mid-animation.
        // Uses triggerMaxDuration (default 8 s) as the reference; falls back to 5 s minimum.
    ...
    private void BeginWakeUp()
    {
        ...
        // Ensure NPC is at the lie position before playing wake-up
        ...
        if (animationController == null) { FinishWakeUpToPatrol(); return; }
    ...
    void PreCollisionRenegotiate()
    {
        ...
        if (useStateMachine && (_state == NPCState.Alerted || _state == NPCState.Attacking
            || _state == NPCState.Talk || _state == NPCState.Sitting || _state == NPCState.Lying))
            return;
    ...
    // Returns true if the NPC is in a body-state that requires standing up first
    private bool IsRecoveringBodyState()
    {
        if (_state == NPCState.Sitting && _sitPhase != SitPhase.None) return true;
        if (_state == NPCState.Lying   && _liePhase != LiePhase.None)  return true;
        return false;
    }
    ...
    void UpdateAnimatorFromMovement()
    {
        ...
        if ((_state == NPCState.Sitting && !inSitWalkup) || (_state == NPCState.Lying && !inLieWalkup))
        {
            vel = Vector3.zero;
            _animVelocitySmoothed = Vector3.zero;
        }
        else if (AgentReady())
        {
            ...
            bool isApproachLike =
                useStateMachine &&
                (_state == NPCState.Approaching || _state == NPCState.Seeking ||
                 _state == NPCState.Patrolling  || inSitWalkup || inLieWalkup);
        ...
        bool inSitIdle = (_state == NPCState.Sitting && !inSitWalkup);
        bool inLieIdle = (_state == NPCState.Lying   && !inLieWalkup);

        if (!IsNavDriven() || inSitIdle || inLieIdle)
        {
            movingX = 0f;
            movingY = 0f;
            blend = 0f;
    ...
    void UpdateFacing()
    {
        ...
        if (useStateMachine && _state == NPCState.Lying   && !inLieWalkup) return;
        ...
        if (to.sqrMagnitude < faceMinDistance * faceMinDistance) return;
