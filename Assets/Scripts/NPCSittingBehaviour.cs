...
public class NPCSittingBehaviour : NPCBehaviourBase
{
    ...
    // Where the NPC stops during Backstepping: seatForward * backstepDistance
    // in front of the seat centre, on the approach side. Never the seat centre
    // itself — that caused the root to clip into the geometry before SitDown.
    private Vector3   _backstepTarget;
    ...
    // seatTf.forward points the same direction as the seated NPC's forward.
    ...
    private bool IsFloorSeat()
    {
        ...
        if (seatDebugLogs)
            Debug.Log($"{name} Sit: seatHeightAboveGround={h:F3}, threshold={floorSitHeightThreshold:F3}, isFloor={isFloor}");
        return isFloor;
    }

    private void TriggerCorrectSitIdle()
    {
        bool useFloorIdle = IsFloorSeat();
        ResetAllAnimatorTriggers();
        if (useFloorIdle)
        {
    ...
    private void TickSearchingSeat(float dt)
    {
        ...
        {
            if (seatDebugLogs) Debug.LogWarning($"{name} Sit: search timed out after {_seatSearchT:F2}s");
            Fail();
            return;
        }
    ...
    private bool TryAcquireSeat()
    {
        ...
        for (int i = 0; i < seats.Length; i++)
        {
            ...
            if (!matchesMask)            { if (seatDebugLogs) Debug.Log($"{name} Sit: REJECT '{s.name}' layer mismatch");     continue; }
            if (!s.IsValid)              { if (seatDebugLogs) Debug.Log($"{name} Sit: REJECT '{s.name}' IsValid=false");      continue; }
            if (s.seatTransform == null) { if (seatDebugLogs) Debug.Log($"{name} Sit: REJECT '{s.name}' seatTransform=null"); continue; }
        ...
        // NPC approaches from in front of the seat (along +seatForward).
        ...
        // Backstep target: backstepDistance in FRONT of the seat centre (+seatForward).
        // The NPC walks from _preSitPoint to here and stops — still clear of the geometry.
        // SitDownLerpRoutine then slides the root into seatedRootOffset as the animation plays.
        Vector3 bsWorld = seatPos2 + seatForward * Mathf.Max(0f, backstepDistance);
        if (!NavMesh.SamplePosition(bsWorld, out NavMeshHit bsHit, ActiveRadius, NavMesh.AllAreas))
            _backstepTarget = _preSitNavPos;
        else
            _backstepTarget = bsHit.position;
        ...
        if (npc.TryFindLadderRoute(_preSitNavPos, out Ladder ladder, out bool goingUp,
            out Vector3 approachPoint, out Vector3 exitPoint, out NavMeshPath ladderPath))
        {
            _routeLadder        = ladder;
        ...
        if (seatDebugLogs) Debug.LogWarning($"{name} Sit: REJECT '{_seat.name}' no direct path and no ladder route");
        ReleaseSeatIfAny();
        ClearLadderRoute();
        return false;
    }
    ...
    {
        if (_routeLadder == null) { if (seatDebugLogs) Debug.LogWarning($"{name} Sit: RoutingToLadder but no ladder."); Fail(); return; }

        if (seatDebugLogs)
            Debug.Log($"{name} Sit: RoutingToLadder hasPath={Agent.hasPath} pending={Agent.pathPending} status={Agent.pathStatus} remaining={Agent.remainingDistance:F2}");

        Agent.isStopped = false;

        if (!Agent.pathPending && Agent.pathStatus == NavMeshPathStatus.PathInvalid)
        { if (seatDebugLogs) Debug.LogWarning($"{name} Sit: ladder approach path became invalid."); Fail(); return; }

        if (!Agent.pathPending && !Agent.hasPath)
        { if (seatDebugLogs) Debug.LogWarning($"{name} Sit: lost ladder approach path."); Fail(); return; }
    ...
    private void OnFinishedLadderRoute()
    {
        ...
        if (!npc.CanReachPosition(_preSitNavPos, out NavMeshPath path)) { if (seatDebugLogs) Debug.LogWarning($"{name} Sit: cannot reach seat after ladder."); Fail(); return; }
    ...
    {
        if (seatDebugLogs)
            Debug.Log($"{name} Sit: ApproachingFront hasPath={Agent.hasPath} pending={Agent.pathPending} status={Agent.pathStatus} remaining={Agent.remainingDistance:F2} dest={Agent.destination} pos={Body.position}");

        Agent.isStopped = false;

        if (!Agent.pathPending && Agent.pathStatus == NavMeshPathStatus.PathInvalid)
        { if (seatDebugLogs) Debug.LogWarning($"{name} Sit: path became invalid while approaching."); Fail(); return; }

        if (!Agent.pathPending && !Agent.hasPath)
        { if (seatDebugLogs) Debug.LogWarning($"{name} Sit: lost path while approaching."); Fail(); return; }
    ...
    private void TickBackstep(float dt, Vector3 seatFacing)
    {
        ...
        // Walk toward _backstepTarget: backstepDistance in front of the seat centre.
        // Stops here before the geometry — lerp carries root the rest of the way.
        float step = Mathf.Max(0.01f, backstepSpeed) * dt;
        Body.position = Vector3.MoveTowards(Body.position, _backstepTarget, step);
    ...
    // Waits until the animator is actually in the SitDown state, then drives
    // the body XZ from normalizedTime so the slide is locked to the clip.
    private IEnumerator SitDownLerpRoutine()
    {
        // Wait until the animator enters SitDown (up to sitTriggerFallbackDelay + a little).
        float waited = 0f;
        float maxWait = sitTriggerFallbackDelay + 0.15f;
        while (waited < maxWait)
        {
            AnimatorStateInfo info = Anim.GetCurrentAnimatorStateInfo(sitAnimLayer);
            if (!string.IsNullOrWhiteSpace(sitDownStateName) && info.IsName(sitDownStateName))
                break;
            waited += Time.deltaTime;
            yield return null;
        }

        // Capture start position at the moment we know the animation is running.
        Vector3 lerpStart = Body.position;
        Vector3 lerpEnd   = _seatTf != null
            ? _seatTf.position + seatedRootOffset
            : Body.position;

        // Drive XZ from normalizedTime until the clip finishes or we leave SitDown.
        while (true)
        {
            if (_seatTf == null) yield break;
            AnimatorStateInfo info = Anim.GetCurrentAnimatorStateInfo(sitAnimLayer);
            bool inSitDown = !string.IsNullOrWhiteSpace(sitDownStateName) && info.IsName(sitDownStateName);
            if (!inSitDown) yield break;   // transitioned to idle — let TickSitDownPlaying handle it
    ...
    private void TickSitDownPlaying(float dt)
    {
        ...
        if (inSitDown && !Anim.IsInTransition(sitAnimLayer) && info.normalizedTime >= 0.95f)
            TriggerCorrectSitIdle();
    }
    ...
    private void TickSittingIdle(float dt, Vector3 seatForward)
    {
        ...
        {
            _seatedT += dt;
            if (_seatedT >= autoStandAfterSeconds) BeginStandUp();
        }
    }
    ...
    private void TickStandUpPlaying(float dt)
    {
        ...
        if (seatDebugLogs)
            Debug.Log($"{name} Sit: StandUpPlaying inStandUp={inStandUp} inSitIdle={inSitIdle} transition={Anim.IsInTransition(sitAnimLayer)} normalized={info.normalizedTime:F2} stateHash={info.fullPathHash}");

        if (_standUpTriggerSent && _standUpT >= sitTriggerFallbackDelay)
        {
            if (!inStandUp && !string.IsNullOrWhiteSpace(standUpStateName))
            {
                if (seatDebugLogs) Debug.Log($"{name} Sit: Fallback Play('{standUpStateName}')");
        ...
        {
            if (seatDebugLogs) Debug.Log($"{name} Sit: StandUp complete by normalizedTime.");
        ...
        {
            if (seatDebugLogs) Debug.LogWarning($"{name} Sit: StandUp exited unexpectedly, forcing finish.");
            FinishStandUpToPatrol();
            return;
        }
        ...
        {
            if (seatDebugLogs) Debug.LogWarning($"{name} Sit: StandUp timeout, forcing finish.");
            FinishStandUpToPatrol();
        }
    }
